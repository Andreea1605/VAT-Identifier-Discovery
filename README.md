...
Manually inspecting the 2 `not_found` results (Breach Farm Energy Storage Limited, Ranksborough Solar Limited) showed the "websites" found were `companieslist.co.uk` and `ukcompanydir.com` - aggregator directories, not the companies' own sites, since these domains were not in my ignore-list filter. This means automated discovery found zero genuine company websites out of 100, not 2.

The block turned out to be temporary, not permanent, but not fully reliable either. After returning to the notebook later and re-running the same pipeline, DuckDuckGo responded normally again for most of the sample: 48 `no_website_found`, 51 `not_found` (a website was found but no VAT number was extracted from the checked pages, genuineness not independently verified), and 1 "candidate_found". However, re-running the same 5-query diagnostic test immediately afterward - using the last 5 companies from that same 100-company run - returned HTTP 202 with 0 results each time, not the normal response. Since these were the last companies processed, right after 100 consecutive queries, this suggests DuckDuckGo's throttling response varies (sometimes a hard 403 block, sometimes a soft 202 with an empty result) but is triggered by the same underlying cause: too many consecutive automated requests, regardless of the exact error code returned.

### Dead End 8: Bing Search Result Extraction Failure
As an alternative to DuckDuckGo, I tested scraping Bing's search results page. The request succeeded (HTTP 200, correct page title), and my CSS selector matched 7 results. However, decoding the actual destination URL from Bing's redirect link revealed it pointed to Bing's own internal image search page (`/images/search?q=...`), not to an external company website.
...
> **Conclusion:** both unauthenticated search-scraping approaches proved unreliable - DuckDuckGo through a temporary but unpredictable IP block, Bing through fragile page structure that never worked at all. Even once DuckDuckGo recovered and produced a usable candidate, that candidate turned out to be a false positive (Dead End 9) - reinforcing that automated discovery at scale needs both a stable paid search API and a robust name-matching step before any result can be trusted.

### Dead End 9: Similar Company Name Collision (Sentinel Labs Ltd)
The automated pipeline found SENTINEL LABS LTD (10486642) a website (`sentinel-laboratories.com`) and extracted VAT candidate GB492426531. HMRC's checker confirmed this number as valid, registered to "SENTINEL LABORATORIES LIMITED" (a different company, number 02231670) at an address in Lindfield, West Sussex. Sentinel Labs Ltd's own filed accounts confirm its actual parent is SentinelOne Inc., a US cybersecurity company - unrelated to Sentinel Laboratories Limited in any way.

> **Why it failed:** unlike the Cruden and Geo Houlton cases, this is not a group-structure false positive - it's a name collision between two entirely unconnected UK companies with similar names. The DuckDuckGo search for "SENTINEL LABS LTD official website" returned the wrong, similarly-named company's site. This is the first false positive produced directly by the automated pipeline, not by manual investigation, and it shows that name similarity alone - without any corporate relationship - can also produce a convincing false positive.
...
The automated discovery pipeline was run twice on the same 100-company sample. The first run was blocked by DuckDuckGo (Dead End 7) and produced 0 genuine company websites. After the block lifted, a second run produced 48 "no_website_found," 51 "not_found" (a website found, but with no VAT number on the checked pages - genuineness not independently verified), and 1 VAT candidate (Sentinel Labs Ltd), which HMRC confirmed belongs to a different, unrelated company (Dead End 9).

### False-positive rate
Measured on the 3 candidates found across the whole project (2 from manual investigation, 1 from the automated pipeline), the false-positive rate was 3 out of 3 (100%). This sample is still small, but includes a false positive produced directly by the automated method, not only by manual investigation - showing the risk applies to both approaches equally.
...
### What this means
This proof of concept did not produce a working, scalable extraction method - it produced strong evidence for why the naive version of this method (search for company name, extract VAT-looking text, treat it as found) does not work, and specifically why it fails in the most dangerous way: by producing plausible, valid-looking, incorrect answers rather than visible gaps. The three confirmed false positives (Cruden, Geo Houlton, Sentinel Labs) are the most important result of this project, because they show precisely the failure mode the original brief warned about - a wrong number is worse than a missing one, and it is invisible unless independently checked against both HMRC and the target company's exact registered details.
